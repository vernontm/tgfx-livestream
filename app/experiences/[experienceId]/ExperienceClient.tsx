...
export default function ExperienceClient({ experienceId, user }: ExperienceClientProps) {
  ...
  const handleStartInstantMeeting = async () => {
    ...
    try {
      ...
      router.push(`/experiences/${experienceId}/live?meetingNumber=${data.meeting.meetingNumber}&password=${data.meeting.password}&title=${encodeURIComponent(data.meeting.title)}`)
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <h3 className="text-lg font-semibold text-white mb-4">
          {meetings.some(m => m.status === 'live') ? 'Live & Past Meetings' : 'Past Meetings'}
        </h3>
              ...
              >
                <div className="p-6">
                  ...
                  <div className="space-y-2 text-sm text-gray-500">
                    <div>Meeting ID: <span className="font-mono">{meeting.zoom_meeting_id}</span></div>
                    {meeting.created_at && (
                  ...
                  <div className="mt-6">
                    <Link
                      href={`/experiences/${experienceId}/meeting/${meeting.id}`}
